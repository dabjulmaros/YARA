//test, must be in node folder
// node -e 'import("./request.cjs").then( loadedModule => loadedModule.request(JSON.stringify({"r":"","params":"","over18":false})).then(r=>console.log(r)))'
const axios = require('axios');
// const adapter =require ('axios/unsafe/adapters/http.js');
const request = async (obj) => {
  const { r, params, over18 } = JSON.parse(obj);
